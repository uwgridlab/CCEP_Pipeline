...
%   Detailed explanation goes here
    ...
    if isempty(startInds) || isempty(endInds) || isempty(maxAmps) || ...
        ...
        goodCell = helpFunc.good_channel_extract(numChans, [], stimChans);
        [startInds,endInds] = analyFunc.get_artifact_indices(data, txt, fsData, ...
            pre, post, goodCell, onsets_samps);
        [templateCell, ~, lengthMax] = analyFunc.get_artifacts(data ,goodCell,...
            startInds, endInds);
        [templateArrayCell, maxIdxArray] = ...
            analyFunc.template_equalize_length(templateCell,data, txt, startInds, ...
            lengthMax);
    ...
    for b1 = 1:length(br)
        ...
        for b2 = 1:length(br{b1})
            ...
            da = analyFunc.template_dictionary(templateArrayCell, data, txt, ...
                dmdb, startInds, endInds, maxLocation, br{b1}{b2});
            txt.Value = vertcat({'--- Extracting data ---'}, txt.Value); pause(0.01);
            txt.Value = vertcat({'--- Checking removal quality ---'}, txt.Value); pause(0.01);
            chck = analyFunc.assess_removal(da, startInds, endInds);
