...
%                 fprintf( 'Training took %0.3f seconds\n',(stop(end-1)*60+stop(end)) - (start(end-1)*60+start(end)) );
        ...
        function get_best_clusters( self )
            ...
            self.bestClusters = HDBSCAN.OFCH( tree.stability,tree.parents );
            
            % find maximum lambda and core points for the best clusters
            [self.corePoints,self.coreLambda] = HDBSCAN.get_core_points( tree.parents,self.bestClusters,full( self.model.lambdaMax ) );
        end  
        ...
        function get_membership( self )
            ...
            tree = self.model.clusterTree;
            self.score = HDBSCAN.GLOSH( self.bestClusters,tree.parents,self.model.lastClust,self.model.lambdaNoise );
            
            % compute labels and probability of cluster membership
            [self.labels,self.P] = HDBSCAN.get_cluster_probability( self.bestClusters,full( self.model.lambdaMax ),self.coreLambda );
